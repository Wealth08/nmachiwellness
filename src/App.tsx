...
function TeaBoxMesh() {
  ...
  const materials = useMemo(() => [
    new THREE.MeshStandardMaterial({ color: '#377532', roughness: 0.4, metalness: 0.08 }),
    new THREE.MeshStandardMaterial({ color: '#377532', roughness: 0.4, metalness: 0.08 }),
    new THREE.MeshStandardMaterial({ color: '#c8c8be', roughness: 0.5, metalness: 0.05 }),
    new THREE.MeshStandardMaterial({ color: '#377532', roughness: 0.5, metalness: 0.05 }),
    new THREE.MeshStandardMaterial({ map: frontTexture, roughness: 0.25, metalness: 0.05 }),
    new THREE.MeshStandardMaterial({ color: '#377532', roughness: 0.4, metalness: 0.08 }),
  ], [frontTexture]);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (json.success) {
        ...
        console.error('Web3Forms rejected:', json);
        setFormStatus('error');
      }
    } catch (err) {
    ...
    <div className="min-h-screen" style={{ fontFamily: "'Bricolage Grotesque', sans-serif", background: BRAND_COLORS.cream, color: BRAND_COLORS.ink }}>
      ...
      <section ref={heroRef} style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', paddingTop: 68, position: 'relative', overflow: 'hidden' }}>
        ...
        <motion.div style={{ y: heroParallax, opacity: heroOpacity, position: 'relative', zIndex: 1, textAlign: 'center', maxWidth: 880, padding: '0 24px', width: '100%' }}>
          ...
          <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.16, duration: 0.75 }}
          ...
          <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.24 }}
            className="grad-text"
      ...
      <section className="py-24 bg-white relative overflow-hidden">
        ...
        <div className="container mx-auto px-6 grid md:grid-cols-2 items-center gap-12">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.85 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.75 }} className="order-1 md:order-2 relative swap-1">
            ...
            <div className="w-full h-[420px] md:h-[620px] cursor-grab active:cursor-grabbing" style={{ userSelect: 'none' }}>
              <Canvas shadows dpr={[1, 2]} gl={{ antialias: true, alpha: true }}>
                ...
                <PresentationControls global config={{ mass: 2, tension: 480 }} snap={{ mass: 4, tension: 1400 }} rotation={[0.08, 0.35, 0]} polar={[-Math.PI / 4, Math.PI / 4]} azimuth={[-Math.PI / 1.6, Math.PI / 1.6]}>
                  <Suspense fallback={null}><TeaBoxMesh /></Suspense>
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 64 }}>
            ...
            <p style={{ fontFamily: 'DM Sans', fontSize: 16, color: BRAND_COLORS.muted, maxWidth: 500, margin: '16px auto 0', lineHeight: 1.7, fontWeight: 300 }}>
              Born in the UK and inspired by centuries of African botanical wisdom, NmaChi Wellness Ltd is bridging the gap between traditional herbalism and modern science.
            </p>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 24 }} className="three-col">
            {[
              ...
              <motion.div key={i} className="glass-card lift-card"
                ...
                <div style={{ width: 56, height: 56, background: 'white', borderRadius: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px', boxShadow: '0 4px 16px rgba(55,117,50,0.1)' }}>{f.icon}</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 26 }} className="three-col">
            {[
              ...
              <motion.div key={i} initial={{ opacity: 0, y: 28 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.12 }}
                ...
                <div style={{ width: 52, height: 52, background: 'white', borderRadius: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', color: BRAND_COLORS.midGreen, marginBottom: 20, boxShadow: '0 4px 12px rgba(55,117,50,0.1)' }}>{t.icon}</div>
            ...
            <div style={{ position: 'relative', width: 340, height: 340 }}>
              <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <div style={{ width: 900, height: 350, background: 'rgba(255,255,255,0.72)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid rgba(151,191,32,0.28)', backdropFilter: 'blur(8px)' }}>
                  <img src="/wtea.png" alt="Wtea" style={{ width: 600, height: 400, objectFit: 'contain' }} className="animate-pulse" />
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="py-24 bg-white">
        <div className="container mx-auto px-6 text-center">
          <motion.div className="glass-card" initial={{ opacity: 0, y: 32 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          ...
          <motion.div className="glass-card" initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            ...
            {formStatus === 'success' ? (
              ...
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
                ...
                {formStatus === 'error' && (
                  <p style={{ fontFamily: 'DM Sans', textAlign: 'center', fontSize: 13, color: '#c0392b', fontWeight: 400, margin: 0 }}>
                    Something went wrong — please try again.
                  </p>
                )}

                <button type="submit" disabled={formStatus === 'submitting'} className="btn-primary"
                  style={{ justifyContent: 'center', borderRadius: 14, padding: '18px' }}>
